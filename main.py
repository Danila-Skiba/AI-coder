import os
import json
import argparse
from chunkers.code_chunker import CodeChunker
from chunkers.doc_chunker import DocChunker

def load_file(filepath):
    with open(filepath, "r", encoding="utf-8") as f:
        return f.read()

def save_chunks(chunks, out_path):
    with open(out_path, "w", encoding="utf-8") as f:
        for chunk in chunks:
            f.write(json.dumps(chunk, ensure_ascii=False) + "\n")

def main(input_dir, output_file):
    code_chunker = CodeChunker()
    doc_chunker = DocChunker()

    all_chunks = []

    # Обходим все файлы в input_dir
    for root, _, files in os.walk(input_dir):
        for filename in files:
            filepath = os.path.join(root, filename)
            ext = os.path.splitext(filename)[1].lower()

            try:
                content = load_file(filepath)
            except Exception as e:
                print(f"Не удалось прочитать файл {filepath}: {e}")
                continue

            if ext in [".py"]:
                chunks = code_chunker.chunk_code(content, filename)
            elif ext in [".md", ".rst", ".txt"]:
                chunks = doc_chunker.chunk_doc(content, filename)
            else:
                # Пропускаем остальные форматы
                continue

            print(f"Обработано {len(chunks)} чанков в файле {filename}")
            all_chunks.extend(chunks)

    # Сохраняем все чанки в JSONL
    save_chunks(all_chunks, output_file)
    print(f"Всего чанков сохранено: {len(all_chunks)}")

def chunk_documents_and_code(input_dir, max_chunk_size=1000):
    code_chunker = CodeChunker()
    doc_chunker = DocChunker()
    all_chunks = []

    for root, _, files in os.walk(input_dir):
        for filename in files:
            filepath = os.path.join(root, filename)
            ext = os.path.splitext(filename)[1].lower()

            try:
                content = load_file(filepath)
            except Exception as e:
                print(f"Не удалось прочитать файл {filepath}: {e}")
                continue

            if ext == ".py":
                chunks = code_chunker.chunk_code(content, filename)
                for c in chunks:
                    c['chunk_type'] = 'code'
            elif ext in [".md", ".rst", ".txt"]:
                chunks = doc_chunker.chunk_doc(content, filename, max_chunk_size)
                for c in chunks:
                    c['chunk_type'] = 'documentation'
            else:
                continue

            all_chunks.extend(chunks)

    return all_chunks

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Чанкирование кода и документации")
    parser.add_argument("--input_dir", type=str, required=True, help="Путь к папке с исходниками и документацией")
    parser.add_argument("--output_file", type=str, required=True, help="Путь к файлу для сохранения чанков JSONL")
    args = parser.parse_args()

    main(args.input_dir, args.output_file)
